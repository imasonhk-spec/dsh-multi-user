...
export function createSupervisor(config, deps = {}) {
   ...
   *
   * `llm-pi-ai:` in the host's `settings.yaml` is the single source of truth for
   * which providers/models exist, so ordinary users never have to configure
   * them. Only that one top-level block is replaced — the account keeps its own
   * UI preferences, onboarding flags and everything else.
   */
  function syncAdminModels(user, home) {
    ...
    const from = join(hostHome, 'settings.yaml')
    if (!existsSync(from)) return { synced: false, reason: 'host settings.yaml is missing' }
    let hostText = ''
    try {
      hostText = readFileSync(from, 'utf8')
    } catch (error) {
      return { synced: false, reason: error.message }
    }
    const block = readTopLevelBlock(hostText, 'llm-pi-ai')
    if (!block) return { synced: false, reason: 'host settings.yaml has no llm-pi-ai block' }
  ...
  async function start(user) {
    ...
    // Bring the account's application set level with the host before booting, so
    // apps installed on the host are present in the instance from the first run.
    ...
    const logPath = join(config.logDir, `${user.username}.log`)
    ...
    const spawnFailure = new Promise((_, rejectPromise) => {
      ...
      child.once('exit', (code, signal) => {
        ...
        rejectPromise(new Error(
          ...
          + `(code ${code}${signal === null ? '' : `, signal ${signal}`}); see ${logPath}`,
        ))
      })
    })
    ...
    const ready = (async () => {
      ...
      for (;;) {
        if (Date.now() > deadline) {
          throw new Error(
            `dsh-multi-user: instance for ${user.username} did not answer within ${config.startTimeoutSeconds}s; see ${logPath}`,
          )
        }
  ...
  const api = {
    ...
    async acquire(user) {
      ...
      if (existing !== undefined && existing.ready) {
        ...
        // started. Mirror it and restart, so the user actually sees the change
        // instead of a page that silently lacks the new app.
        if (config.syncProfileOnStart && (await syncProfile(user)).changed === true) {
