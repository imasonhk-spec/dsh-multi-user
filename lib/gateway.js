...
import { normalizeUi, verifyPassword } from './store.js'
...
export function createGateway(options) {
  ...
  async function handleOwnRoute(req, res, url) {
    ...
    // The floating sign-out pill is draggable, so every signed-in account
    // needs a place to park its position. This sits ABOVE the administrator
    // fence on purpose: it is a preference of the caller, not a user record.
    // The account comes from the session cookie, never from the request body,
    // so one account can never write another's position.
    // The floating sign-out pill is draggable and collapsible, so every
    // signed-in account needs a place to park that state. This sits ABOVE the
    // administrator fence on purpose: it is a preference of the caller, not a
    // user record. The account comes from the session cookie, never from the
    // request body, so one account can never write another's state.
    if ((relative === '/api/ui-prefs' || relative === '/api/ui-prefs/')
      && (method === 'GET' || method === 'POST')) {
      if (identity === undefined) return sendJson(res, 401, { error: 'unauthorized' })
      if (method === 'GET') {
        const ui = normalizeUi(identity.user.ui)
        return sendJson(res, 200, { pill: ui.pill ?? null, collapsed: ui.collapsed === true })
      }
      let parsed
      try {
        parsed = JSON.parse((await readBody(req, 8 * 1024)) || '{}')
      } catch {
        return sendJson(res, 400, { error: '请求体不是合法 JSON' })
      }
      const body = parsed ?? {}
      // `pill: null` is a real instruction ("put it back to the default
      // corner"), so it must survive the merge — only an ABSENT key means
      // "leave the position alone". Spreading blindly would turn an explicit
      // null into a no-op and the double-click reset would silently stop
      // working.
      const next = { ...identity.user.ui }
      if ('pill' in body) next.pill = body.pill
      if ('collapsed' in body) next.collapsed = body.collapsed
      const ui = normalizeUi(next)
      const saved = await store.update(identity.user.username, { ui })
      if (saved.error !== undefined) return sendJson(res, 400, { error: saved.error })
      return sendJson(res, 200, { ok: true, pill: ui.pill ?? null, collapsed: ui.collapsed === true })
    }
    ...
    const upstreamReq = httpRequest(options, (upstreamRes) => {
      ...
      upstreamRes.on('end', () => {
        ...
        if (wantsFence) {
          ...
          const widget = logoutWidget(base, user.username, user.role, normalizeUi(user.ui).pill, normalizeUi(user.ui).collapsed === true)
  ...
  async function onRequest(req, res) {
    ...
    try {
      // Sliding session renewal: when the signed-in session is past the renewal
      // threshold, extend it server-side and re-issue the session/user cookies on
      // THIS response - whichever path (own route or proxied upstream answer)
      // ends up writing the headers. Without the cookie re-issue the browser
      // would still drop the cookie at its original Max-Age and every later
      // client-api call would surface as `transport failure ... HTTP 401`.
      {
        const identityNow = currentUser(req)
        if (identityNow !== undefined) {
          const renewedExpiresAt = sessions.renewIfDue(identityNow.token)
          if (renewedExpiresAt !== undefined) {
            const freshCookies = [
              sessionCookie(identityNow.token, renewedExpiresAt),
              userCookie(identityNow.user.username, renewedExpiresAt),
            ]
            const originalWriteHead = res.writeHead.bind(res)
            res.writeHead = function patchedWriteHead(status, headers) {
              const merged = { ...(headers ?? {}) }
              const existing = merged['set-cookie'] ?? merged['Set-Cookie']
              merged['set-cookie'] = existing === undefined
                ? freshCookies
                : [...(Array.isArray(existing) ? existing : [existing]), ...freshCookies]
