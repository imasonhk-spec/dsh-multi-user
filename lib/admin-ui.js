...
export function logoutWidget(basePath, username, role, initialPill, initialCollapsed) {
  const payload = JSON.stringify({
    basePath,
    username,
    // The single character shown on the collapsed avatar. Computed here so the
    // client does not have to reason about surrogate pairs or locale quirks.
    initial: ((username || '?').trim().charAt(0) || '?').toUpperCase(),
    // Every account gets a "管理" entry, but they do NOT lead to the same page:
    //   admin  -> /admin   (the user-management console)
    //   others -> /account (their own profile, available apps, password)
    // Resolved here on the server so the client never has to know the roles,
    // and so a normal account can never be handed the administrator URL.
    manageHref: role === 'admin' ? `${basePath}/admin` : `${basePath}/account`,
    prefsUrl: `${basePath}/api/ui-prefs`,
    parked: initialPill ?? null,
    // Remembering the collapsed flag means a reload restores whatever the user
    // last looked at, instead of always springing back open.
    collapsed: initialCollapsed === true,
  })
  return `<script data-dsh-multi-user="widget">(()=>{const C=${payload};
const M=document.createElement('div');M.setAttribute('data-dsh-multi-user','pill');
M.style.cssText='position:fixed;z-index:2147483000;right:14px;bottom:14px;left:auto;top:auto;font:12px/1.4 -apple-system,BlinkMacSystemFont,Segoe UI,Noto Sans SC,sans-serif';
// Hide the host until its shadow content has been laid out and positioned.
// Without this the pill paints once in the CSS default corner and then jumps to
// its restored spot; and when the layout is being pre-measured for an expand
// animation we do NOT want the intermediate frame on screen at all.
M.style.visibility='hidden';
const S=M.attachShadow({mode:'open'});
S.innerHTML='<style>*{box-sizing:border-box}'
+'div.w{display:flex;align-items:center;gap:8px;background:rgba(28,31,35,.86);color:#eceff3;border:1px solid rgba(255,255,255,.14);border-radius:999px;padding:5px 6px 5px 6px;backdrop-filter:blur(8px);box-shadow:0 6px 22px rgba(0,0,0,.28)}'
+'span.g{cursor:grab;user-select:none;-webkit-user-select:none;padding:0 2px;opacity:.55;letter-spacing:1px;font-size:11px;line-height:1;touch-action:none}'
+'span.g:active{cursor:grabbing}'
+'div.w:hover span.g{opacity:.9}'
+'span.t{white-space:nowrap}b{font-weight:650}'
+'button{font:inherit;cursor:pointer;background:rgba(255,255,255,.14);color:#fff;border:1px solid rgba(255,255,255,.18);border-radius:999px;padding:3px 10px}'
+'button:hover{background:rgba(255,255,255,.24)}'
+'a{color:#9dc0ff;text-decoration:none}a:hover{text-decoration:underline}'
+'div.w.dragging{cursor:grabbing;box-shadow:0 10px 30px rgba(0,0,0,.42)}'
// The collapsed avatar: a round button carrying the account's initial only.
+'button.c{display:none;align-items:center;justify-content:center;gap:0;padding:0;width:36px;height:36px;border-radius:50%;background:rgba(28,31,35,.86);border:1px solid rgba(255,255,255,.16);box-shadow:0 6px 22px rgba(0,0,0,.28);font-weight:650;font-size:14px;line-height:1;touch-action:none}'
+'button.c:hover{background:rgba(48,52,58,.94)}'
+'button.c .i{letter-spacing:0}'
+'div.w.dragging,button.c.dragging{cursor:grabbing;box-shadow:0 10px 30px rgba(0,0,0,.42)}'
+'</style>'
+'<div class="w"><span class="g" title="拖动以移动">⠿</span><span class="t">已登录 <b></b></span>'
+'<a class="m" target="_self">管理</a>'
+'<button type="button" class="q">退出</button></div>'
+'<button type="button" class="c" title="点击展开"><span class="i"></span></button>';
const W=S.querySelector('div.w'), GRIP=S.querySelector('span.g');
// While collapsed the host div IS the round avatar, so it also has to carry the
// circle styling (border-radius in the host's own inline style, which a shadow
// style rule cannot reach).
const BUTTON=S.querySelector('button.c');
S.querySelector('a.m').setAttribute('href',C.manageHref);
S.querySelector('b').textContent=C.username;
S.querySelector('span.i').textContent=C.initial;
S.querySelector('button.q').addEventListener('click',()=>{location.href=C.basePath+'/logout'});

const EDGE=14;
// How long the pill stays open after the pointer goes quiet, before it tidies
// itself back into the round avatar.
const COLLAPSE_MS=2500;
// How long a drag may sit motionless before we stop treating it as "active".
const IDLE_MS=900;
// The widget is positioned with left/top ONLY. The initial CSS corner is
// right/bottom, so it is converted to an explicit left/top right after mount —
// every helper below can then read one unambiguous coordinate pair, with no
// "which corner am I anchored to" special cases.
const ANIM='left .18s ease,top .18s ease,width .18s ease,height .18s ease,background .18s ease,border-radius .18s ease,visibility 0s linear 0s';
function view(){return {w:window.innerWidth,h:window.innerHeight};}
// Measure whichever shape is currently on screen. The host div is
// position:fixed with no intrinsic size, so it always reports zero before the
// shadow content has been laid out — the inner element carries the real box.
function shape(){return isCollapsed()?BUTTON:W;}
function box(){const r=shape().getBoundingClientRect();
  return {w:Math.round(r.width),h:Math.round(r.height)};}
function clamp(n,lo,hi){return n<lo?lo:n>hi?hi:n;}
function isCollapsed(){return M.getAttribute('data-state')==='collapsed';}
function curX(){const l=M.style.left;if(l&&l!=='auto'){const n=parseFloat(l);if(isFinite(n))return n;}const r=parseFloat(M.style.right);return isFinite(r)?view().w-r-box().w:0;}
function curY(){const n=parseFloat(M.style.top);return isFinite(n)?n:0;}
// Write the widget's top-left corner. Only ever one axis at a time is touched
// by the callers, but writing both keeps the invariant simple.
function put(left,top){
  M.style.left=left+'px'; M.style.top=top+'px';
  M.style.right='auto'; M.style.bottom='auto';}
// Anchor to an edge WITHOUT trusting the (sometimes transient) capsule width:
// a right-dock pins right:EDGE so the margin is exact regardless of layout or
// an in-flight transition. A left-dock keeps the absolute left.
function dockEdge(side,top){
  M.style.transition=ANIM; M.style.top=top+'px'; M.style.bottom='auto';
  if(side==='left'){M.style.left=EDGE+'px'; M.style.right='auto';}
  else{M.style.right=EDGE+'px'; M.style.left='auto';}}
// Free positioning used BOTH while dragging and when settling. The side is an
// OUTPUT (which edge we ended up nearer), never an input that silently cancels
// horizontal movement — pinning mid-drag was what made the widget only appear
// to move vertically.
function freePlace(cx,cy){
  const b=box(),v=view();
  const top=clamp(cy-b.h/2,EDGE,v.h-b.h-EDGE);
  // Keep a grabbable sliver on screen; the full edge snap happens on release.
  const left=clamp(cx-b.w/2,EDGE-b.w+24,v.w-24);
  put(left,top);}
function sideNow(){const b=box(),v=view();return (curX()+b.w/2)<v.w/2?'left':'right';}
// Snap to the nearer edge, keeping the vertical position.
function settle(){const b=box(),v=view(),cy=curY()+b.h/2,s=sideNow();
  dockEdge(s,cy);}
function applyCollapsedChrome(on){
  M.setAttribute('data-state',on?'collapsed':'open');
  W.style.display=on?'none':'';
  BUTTON.style.display=on?'flex':'none';
  // The host wraps whichever shape is showing, so mirror its geometry.
  M.style.borderRadius=on?'50%':'999px';
  M.style.background=on?'transparent':'';
  M.style.border=on?'none':'';
  M.style.boxShadow=on?'none':'';
}
function collapse(){if(isCollapsed())return;clearTimeout(collapseTimer);
  const cy=curY()+box().h/2;
  M.style.visibility='hidden';
  applyCollapsedChrome(true);
  const nb=box();
  // Re-centre on the same anchor so the round avatar appears to grow out of the
  // pill's middle rather than sliding in from a corner.
  M.style.transition=ANIM;
  freePlace(curX()+nb.w/2,cy);
  M.style.visibility='';
  save({collapsed:true});}
function expand(){if(!isCollapsed())return;clearTimeout(collapseTimer);
  const cy=curY()+box().h/2;
  M.style.visibility='hidden';
  applyCollapsedChrome(false);
  // Measuring the open layout here is what makes the width/height animation
  // start from a real box instead of from zero.
  const nb=box(),v=view();
  M.style.transition=ANIM;
  // The centre stays put; the pill grows back inward from its own edge.
  const s=(curX()+nb.w/2)<v.w/2?'left':'right';
  dockEdge(s,clamp(cy-nb.h/2,EDGE,v.h-nb.h-EDGE));
  M.style.visibility='';
  armCollapse();}
function armCollapse(delay){clearTimeout(collapseTimer);
  collapseTimer=setTimeout(()=>{if(!dragging)collapse();},delay===undefined?COLLAPSE_MS:delay);}
let collapseTimer=null,resizeTimer=null;

// ── drag ────────────────────────────────────────────────────────────────
// Both the open pill's grip AND the collapsed avatar are draggable, so the
// round button keeps the "move me anywhere" promise.
let dragging=false,sx=0,sy=0,ox=0,oy=0,moved=false,pointerId=null,grabbed=null,idleTimer=null;
function armIdle(){clearTimeout(idleTimer);
  idleTimer=setTimeout(()=>{if(dragging&&!collapseTimer)armCollapse(0);},IDLE_MS);}
function startDrag(e,el){if(e.button!==undefined&&e.button!==0)return;
  e.preventDefault(); dragging=true; moved=false; pointerId=e.pointerId; grabbed=el;
  try{el.setPointerCapture(pointerId);}catch{}
  sx=e.clientX; sy=e.clientY;
  // Anchor the drag on the VISIBLE shape's own box. The host div is
  // position:fixed with no intrinsic size, so its rect is not a reliable
  // reference and the widget would lag behind the pointer by its own offset.
  const r=(isCollapsed()?BUTTON:W).getBoundingClientRect();
  ox=r.left; oy=r.top;
  M.style.transition='';            // no easing while being dragged
  W.classList.add('dragging'); BUTTON.classList.add('dragging');
  armIdle();}
function moveDrag(e){if(!dragging||e.pointerId!==pointerId)return;
  e.preventDefault();
  const dx=e.clientX-sx, dy=e.clientY-sy;
  if(!moved&&Math.abs(dx)+Math.abs(dy)<3)return; moved=true;
  M.style.transition='';            // a drag must track the pointer exactly
  freePlace(ox+dx+box().w/2,oy+dy+box().h/2);
  armIdle();}
function endDrag(e){if(!dragging)return; if(pointerId!==null&&e.pointerId!==pointerId)return;
  dragging=false; clearTimeout(idleTimer); idleTimer=null;
  W.classList.remove('dragging'); BUTTON.classList.remove('dragging');
  try{grabbed&&grabbed.releasePointerCapture(pointerId);}catch{} pointerId=null; grabbed=null;
  if(!moved){armCollapse();return;}       // a tap on the grip is not a move
  // Only NOW do we choose an edge, and ease into it.
  settle();
  const b=box(),v=view(),cy=curY()+b.h/2,s=sideNow();
  // Park by centre-fraction so the spot survives a resize or a different
  // screen without drifting into a corner.
  save({pill:{x:s==='left'?EDGE/v.w:(v.w-EDGE)/v.w,y:cy/v.h,side:s}});
  // Once the pointer goes quiet the pill tidies itself away.
  armCollapse();}
GRIP.addEventListener('pointerdown',(e)=>startDrag(e,GRIP));
GRIP.addEventListener('pointermove',moveDrag);
GRIP.addEventListener('pointerup',endDrag);
GRIP.addEventListener('pointercancel',endDrag);
BUTTON.addEventListener('pointerdown',(e)=>startDrag(e,BUTTON));
BUTTON.addEventListener('pointermove',moveDrag);
BUTTON.addEventListener('pointerup',endDrag);
BUTTON.addEventListener('pointercancel',endDrag);
// A click (as opposed to a drag) on the round avatar opens the pill again.
BUTTON.addEventListener('click',(e)=>{if(moved)return;e.preventDefault();e.stopPropagation();expand();});

// Double-click the pill body resets to the bottom-right default.
W.addEventListener('dblclick',(e)=>{e.preventDefault();reset();});
function reset(){clearTimeout(collapseTimer);
  M.style.transition='';
  applyCollapsedChrome(false);
  const b=box(),v=view();
  M.style.right='auto'; M.style.bottom='auto';
  dockEdge('right',v.h-b.h-EDGE);
  C.parked=null;save({pill:null,collapsed:false});
  armCollapse();}
// Send only what changed: a null pill clears the parked spot, an absent key
// leaves it alone. The server mirrors that contract.
function save(patch){try{fetch(C.prefsUrl,{method:'POST',credentials:'same-origin',
  headers:{'content-type':'application/json'},body:JSON.stringify(patch||{})}).catch(()=>{});}catch{}}

// Keep it reachable when the window shrinks or the screen rotates.
window.addEventListener('resize',()=>{if(resizeTimer)clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>{
  M.style.transition='';
  // Re-anchor around the same centre: the widget is always at an explicit
  // left/top by now, so this is a plain re-clamp plus a re-snap.
  const b=box(),v=view(),cy=curY()+b.h/2;
  settle();
  const nb=box(),ncy=curY()+nb.h/2,s=sideNow();
  save({pill:{x:s==='left'?EDGE/v.w:(v.w-EDGE)/v.w,y:ncy/v.h,side:s}});
  if(!isCollapsed())armCollapse();},180);});

document.body.appendChild(M);
// Only now does the widget have a real box to measure, so restore last.
(function restore(){
  M.style.transition='';                  // no animation on first paint
  applyCollapsedChrome(C.collapsed===true);
  const b=box(),v=view(),p=C.parked;
  if(p&&typeof p.x==='number'&&typeof p.y==='number'){
    // Stored as a centre fraction + the edge it was docked to.
    dockEdge(p.side==='left'?'left':'right',clamp(p.y*v.h-b.h/2,EDGE,v.h-b.h-EDGE));
  } else {
    // No stored spot: adopt the CSS default corner as explicit coordinates so
    // every later helper can read a single unambiguous left/top pair.
    dockEdge('right',v.h-b.h-EDGE);
  }
  M.style.visibility='';
  if(!C.collapsed)armCollapse();            // an open pill tidies itself away
})();})();</script>`
}
