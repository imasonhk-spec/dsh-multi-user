...
export async function apply(ctx, rawConfig = {}) {
  ...
  const supervisor = createSupervisor(config, {
    ...
    /**
     * The administrator account's own harness home, when it runs managed.
     *
     * Their `settings.yaml` is where 设置 → 模型 actually writes, so it — not
     * the host file that merely seeded it — is what every other account's model
     * catalogue is copied from. A host-mode administrator keeps using the host
     * home, and the caller falls back to it when this returns undefined.
     */
    adminHome: () => {
      const snapshot = store.snapshot()
      const admin = (snapshot.users ?? []).find((entry) => (
        String(entry.role ?? '').toLowerCase() === 'admin'
        && entry.homeMode !== 'host'
        && typeof entry.homeDir === 'string'
        && entry.homeDir.length > 0
      ))
      return admin?.homeDir
    },
